...
typedef struct _MESH_INFO_TYPE {
	MPI_Comm Comm;
	MPI_Comm Row_comm;
	MPI_Comm Col_comm;
	int Row;
...
/* Communication block set up for mesh topology */
...
int main(int argc, char *argv[])
{
	...
	/* Reading Input */
	if (grid.MyRank == Root) {
		if ((fopen_s(&fp, "./data/mdata1.inp", "r")) == NULL) {
		...
		if (MatrixA_FileStatus != 0) {
			...
			for (irow = 0; irow < NoofRows_A; irow++) {
				...
				for (icol = 0; icol < NoofCols_A; icol++)
					fscanf_s(fp, "%f", &Matrix_A[irow][icol]);
			}
			fclose(fp);
		}

		if ((fopen_s(&fp, "./data/mdata2.inp", "r")) == NULL) {
		...
		if (MatrixB_FileStatus != 0) {
			...
			for (irow = 0; irow < NoofRows_B; irow++) {
				...
				for (icol = 0; icol < NoofCols_B; icol++)
					fscanf_s(fp, "%f", &Matrix_B[irow][icol]);
			}
			fclose(fp);
		}
